...
export function selectFolder() {
  return new Promise((resolve, reject) => {
    ...
    exec(script, (error, stdout) => {
      if (error)
        return reject(new Error('Folder selection cancelled or failed'));
...
export const rmWithRetry = (targetPath) => {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (attempt === 2) {
        console.error(
          `[ClearTrash] Failed to delete ${targetPath}: ${e.message}`,
        );
      }
    }
  }
    ...
    for (const trashFile of TRASH_FILES) {
      ...
      if (fs.existsSync(filePath)) {
        try {
          ...
          console.error(
            `[ClearTrash] Failed to delete ${filePath}: ${e.message}`,
          );
        }
      }
    }
  ...
  for (const entry of entries) {
    if (entry.startsWith('debug_') && entry.endsWith('.png')) {
      ...
      try {
        ...
        console.error(
          `[ClearDebug] Failed to delete ${filePath}: ${e.message}`,
        );
      }
    }
  }
...
export function clearTrashDirectory(trashDir) {
  ...
  if (!fs.existsSync(trashDir))
    return { success: true, deletedEntries: 0, freedBytes: 0, freedMB: 0 };
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        console.error(
          `[ClearTrashDir] Failed to remove ${fullPath}: ${e.message}`,
        );
      }
    }
  } catch (_) {}

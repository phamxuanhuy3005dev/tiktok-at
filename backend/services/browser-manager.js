...
export async function releaseProfileLocks(userDataDir, profileName) {
  ...
  if (pids.length > 0) {
    console.log(
      `[${profileName}] Found ${pids.length} lingering Chromium processes for profile. Terminating...`,
    );
...
export async function closeProfileBrowser(profileId, profileName) {
  if (manualBrowsers.has(profileId)) {
    ...
    try {
      await browser.close();
      console.log(
        `[${profileName}] Manual browser context closed successfully`,
      );
    } catch (e) {
...
export async function ensureProfileReadyForLaunch(profileId, profileName) {
  ...
  if (manualBrowsers.has(profileId)) {
    console.log(
      `[${profileName}] Active manual browser detected. Closing it before launch...`,
    );
...
export async function launchPersistentContextSafe(
  ...
  try {
    ...
    if (isLockError) {
      console.warn(
        `[${profileName}] Browser launch collision detected: "${msg}". Force-releasing locks and retrying...`,
      );
      await releaseProfileLocks(userDataDir, profileName);

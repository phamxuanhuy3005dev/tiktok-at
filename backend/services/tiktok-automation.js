...
import { db, DUMMY_VIDEOS_DIR, PROFILES_DIR } from '../db.js';
import {
...
import { launchPersistentContextSafe, releaseProfileLocks } from './browser-manager.js';
import { injectProfileCookies } from './cookie-service.js';
import { addingFavoriteMusicProfiles, loggingInProfiles } from './tracker.js';
...
export async function resolveScheduleInputs(page, log) {
  ...
  log(
    `Visible schedule inputs: ${orderedInputs.map(describeScheduleInput).join(' | ')}`,
  );
  ...
  if (!selected.time) {
    selected.time =
      orderedInputs.find((input) => input.index !== selected.date?.index) ||
      orderedInputs[1] ||
      null;
  }
        ...
        try {
          ...
          if (
            ...
            if (cancelBtn && (await cancelBtn.isVisible())) {
              ...
              console.log(
                '[dismissPopups] Dismissed "Turn on automatic content checks" popup -> Cancel',
              );
              return true;
            }
          }
          ...
          if (
            text.includes('Discard this post') ||
            text.includes('discarded permanently')
          ) {
            const notNowBtn = await modal.$(
              'button:has-text("Not now"), button:has-text("Cancel")',
            );
            if (notNowBtn && (await notNowBtn.isVisible())) {
              await notNowBtn.click();
              console.log(
                '[dismissPopups] Dismissed "Discard this post?" popup -> Not now',
              );
              return true;
            }
          ...
          if (text.includes('Phone mode')) {
            ...
            if (gotItBtn && (await gotItBtn.isVisible())) {
              ...
              console.log(
                '[dismissPopups] Dismissed "Phone mode" tutorial modal -> Got it',
              );
              return true;
            }
          }
          ...
          for (const btnSel of genericBtnSelectors) {
            ...
            if (btn && (await btn.isVisible())) {
              await btn.click();
              console.log(
                `[dismissPopups] Dismissed generic modal -> ${btnSel}`,
              );
              return true;
            }
          }
...
export async function dismissOnboardingModals(page, log) {
  try {
    const detection = await page.evaluate(() => {
      ...
      if (tuxModal) {
        ...
        if (
          ...
          const btns = Array.from(tuxModal.querySelectorAll('button')).map(
      ...
      const tutorialTips = document.querySelectorAll(
        '[class*="tutorial-tooltip"], .react-joyride__tooltip',
      );
      for (const tip of tutorialTips) {
        ...
        if (
          ...
          const btns = Array.from(tip.querySelectorAll('button')).map((b) =>
            b.textContent.trim(),
          );
      ...
      const joyrides = document.querySelectorAll(
        '[class*="joyride"], [data-joyride]',
      );
      for (const jr of joyrides) {
        ...
        if (
          ...
          const btns = Array.from(jr.querySelectorAll('button')).map((b) =>
            b.textContent.trim(),
          );
    ...
    if (log)
      log(
        `Detected ${detection.length} onboarding modal(s): ${detection.map((d) => d.type + '(' + (d.buttons ? d.buttons.join(',') : '') + ')').join('; ')}`,
      );

    // Handle TUXModal Cancel/Dismiss (never Discard)
    try {
      const tuxCancel = await page
        .locator(
          'div.TUXModal button:has-text("Cancel"), div.TUXModal button:has-text("No"), div.TUXModal button:has-text("Not now")',
        )
        .first();
...
export async function checkExistingScheduledTime(page, log) {
  ...
  try {
    ...
    await page
      .waitForSelector(
        'button:has-text("Posts"), [role="tab"]:has-text("Posts"), [role="tablist"], [data-tt*="PostTable"], .post-table',
        { timeout: 6000 },
      )
      .catch(() => null);
    ...
    const hasScheduledTab = await scheduledTab
      .isVisible({ timeout: 1500 })
      .catch(() => false);

    if (!hasScheduledTab) {
      log(
        `No "Scheduled" tab found on content page (0 scheduled posts on this account). Proceeding directly to upload.`,
      );
      return null;
    }
    ...
    const latestTime = await page.evaluate(() => {
      ...
      const postTable = document.querySelector(
        '[data-tt*="PostTable"], [class*="PostTable"]',
      );
      if (postTable) {
    ...
    if (latestTime) {
      ...
      const parsed = new Date(
        latestTime.replace(/(?:Scheduled for|Scheduled:?)\s*/, ''),
      );
    ...
    log(
      `Error checking scheduled videos: ${e.message}. Proceeding with default schedule.`,
    );
    return null;
  }
}
...
export async function uploadVideo(
  ...
  if (!videos || videos.length === 0) {
    console.log(
      `[${profile.name}] No compatible videos found in ${videoFolder}. Skipping.`,
    );
    return 0;
  }
  ...
  try {
    ...
    if (profile.auto_increment_schedule) {
      ...
      if (existingTime) {
        ...
        log(
          `Existing schedule detected. ALL ${Math.min(videos.length, maxUploads)} videos will be scheduled from base: ${existingTime.toISOString()}`,
        );
      }
    }
    // --- End content check ---
    ...
    for (let i = 0; i < videos.length; i++) {
        ...
        try {
          ...
          for (let poll = 0; poll < 30; poll++) {
            const [hasInput, hasButton, isLogin] = await Promise.all([
              ...
              page.$(
                'button.upload-stage-btn, .upload-stage-btn, [data-e2e="upload-video-button"]',
              ),
      ...
      if (!uploaded)
        throw new Error('Could not find file input or upload button');
      ...
      try {
        const uploadCompletedPromise = (async () => {
          ...
          try {
            await uploadProgressCancel.waitFor({
              state: 'visible',
              timeout: 10000,
            });
            cancelDetected = true;
      ...
      try {
        ...
        await page.waitForSelector(
          '.video-info-container, textarea, .DraftEditor-root, .editor-entrance, [data-button-name="sounds"], button:has-text("Post")',
          { timeout: 60000 },
        );
        ...
        if (shouldRemoveTitle) {
          ...
          const captionSelectors = [
            ...
            'textarea',
            '[role="textbox"]',
            '[data-e2e="caption-edit-container"]',
          ];
      ...
      if (useSetMusic) {
        try {
          log(
            `Waiting for video upload to settle before opening Sounds editor...`,
          );
          ...
          if (soundsVisible) {
            ...
            const hasMusicSearch =
              profile.music_search && profile.music_search.trim();

            if (hasMusicSearch) {
              // Chế độ 1: Tìm kiếm theo danh sách từ khóa
              const keywords = profile.music_search
                .split(',')
                .map((k) => k.trim())
                .filter(Boolean);
              const currentKeyword = keywords[uploadedCount % keywords.length];
              log(
                `Search music mode: searching for "${currentKeyword}" (${(uploadedCount % keywords.length) + 1}/${keywords.length})...`,
              );

              const searchInput = page
                .locator(
                  'input.TextInput__input[placeholder*="Search sounds" i], input[placeholder*="Search sounds" i]',
                )
                .first();
              if (
                await searchInput
                ...
                const searchPlusBtns = page.locator(
                  '.Button__root--shape-rounded, button[class*="Button__root--shape-rounded"]',
                );
                const count = await searchPlusBtns.count();
                if (count > 0) {
                  log(
                    `Found ${count} sound result(s) for "${currentKeyword}". Adding top sound...`,
                  );
                  await searchPlusBtns.first().click({ force: true });
                  soundAdded = true;
                } else {
                  log(
                    `No sound results found for "${currentKeyword}". Falling back to Favorites...`,
                  );
                }
              }
            }
            ...
            if (!soundAdded) {
              ...
              const favTab = page
                ...
                .first();
              if (
                await favTab.isVisible({ timeout: 5000 }).catch(() => false)
              ) {
              ...
              const favPlusBtns = page.locator(
                '.Button__root--shape-rounded, button[class*="Button__root--shape-rounded"]',
              );
              ...
              if (totalFavs > 0) {
                ...
                log(
                  `Music rotation: video #${uploadedCount + 1} -> picking favorite #${targetIdx + 1}/${totalFavs}`,
                );
                await favPlusBtns.nth(targetIdx).click({ force: true });
                soundAdded = true;
              } else {
                log(
                  `No favorites found. Trying first sound from For You / default list...`,
                );
                const anyPlusBtn = page
                  .locator('.Button__root--shape-rounded')
                  .first();
                if (
                  await anyPlusBtn
                    .isVisible({ timeout: 3000 })
            ...
            if (soundAdded) {
              log(
                `Sound added to timeline. Waiting for Audio property panel...`,
              );
              await page.waitForTimeout(2000);

              // Set Volume to -50 dB in Audio properties panel
              try {
                const propInput = page
                  .locator(
                ...
                if (
                  ...
                  log(
                    `Audio PropSettingInput not visible. Keeping default volume.`,
                  );
                }
              } catch (volErr) {
              ...
              if (
                await saveBtn.isVisible({ timeout: 5000 }).catch(() => false)
              ) {
                await saveBtn.click();
                log(`Clicked Save button. Waiting to return to upload form...`);
                await page.waitForSelector(
                  'button[data-e2e="post_video_button"], button:has-text("Post")',
                  { timeout: 30000 },
                );
                await page.waitForTimeout(2000);
            ...
            log(
              `Sounds button (.editor-entrance[data-button-name="sounds"]) not visible. Skipping editor steps.`,
            );
          }
        } catch (e) {
            ...
            await page
              .screenshot({
                path: path.join(
                  __dirname,
                  '..',
                  `debug_${profile.name}_sound_fail.png`,
                ),
              })
              .catch(() => null);
          }
      ...
      if (profile.need_content_check !== 0) {
        try {
          ...
          const headline = page.locator('.headline-wrapper', {
            hasText: 'Content check lite',
          });
          ...
          if ((await headline.count()) > 0) {
            ...
            if ((await switchContent.count()) > 0) {
              const isChecked =
                (await switchContent.getAttribute('data-state')) ===
                  'checked' ||
                (await switchContent.getAttribute('aria-checked')) === 'true';
              if (!isChecked) {
              ...
              while (Date.now() - checkStartTime < maxCheckTime) {
                const status = await page.evaluate(() => {
                  ...
                  if (
                    successEl &&
                    successEl.getAttribute('data-show') === 'true'
                  ) {
                    return 'success';
                  }
                  ...
                  const checkingEl = document.querySelector(
                    '.status-result.status-checking',
                  );
                  if (
                    checkingEl &&
                    checkingEl.getAttribute('data-show') === 'true'
                  ) {
                    return 'checking';
                  }
                  const readyEls = Array.from(
                    document.querySelectorAll('.status-result.status-ready'),
                  );
                  const visibleReady = readyEls.find(
                    (el) => el.getAttribute('data-show') === 'true',
                  );
                  if (visibleReady) {
                ...
                if (status === 'success' || status === 'limit_reached') {
                  if (status === 'limit_reached') {
                    log(
                      'Daily content check limit reached. Proceeding to post without safety check validation.',
                    );
                  }
                  checkSuccess = true;
                  break;
                  ...
                  if (
                    !toggledRetry &&
                    Date.now() - checkStartTime > 3 * 60 * 1000
                  ) {
                    log(
                      'Stuck in checking for 3 minutes. Toggling Content check lite off and on again...',
                    );
                    try {
                      await switchContent.click({ force: true });
                      await page.waitForTimeout(1000);
                      ...
                      log(
                        `Failed to toggle retry switch: ${toggleErr.message}`,
                      );
                    }
                  }
                  ...
                  log(
                    `Content check failed/restricted/warned/errored with status: ${status}`,
                  );
                  break;
                }
              }
      ...
      if (profile.auto_increment_schedule) {
        try {
          ...
          if (hasExistingSchedule) {
            ...
            await scheduleRadioInput.waitFor({
              timeout: 15000,
              state: 'attached',
            });
            ...
            lastScheduledTime = computeAutoIncrementTime({
              lastScheduledTime,
              intervalMinutes: intervalMin,
            });
            const dateValue = formatScheduleValue(
              lastScheduledTime,
              'date',
              scheduleInputs.date || {},
            );
            const timeValue = formatScheduleValue(
              lastScheduledTime,
              'time',
              scheduleInputs.time || {},
            );
            ...
            log(
              `Video ${i + 1}: Scheduling at ${dateValue} ${timeValue} (from existing batch).`,
            );
            await fillScheduleInput(
              page,
              scheduleInputs.date,
              dateValue,
              'Date',
              ...
              await page
                .screenshot({
                  path: path.join(
                    __dirname,
                    '..',
                    `debug_${profile.name}_autoincrement_${i + 1}.png`,
                  ),
                })
                .catch(() => null);
            }
              ...
              if (text) {
                ...
                for (const pattern of idPatterns) {
                  ...
                  if (match && match[1]) {
                    ...
                    log(
                      `Captured video ID from API: ${capturedVideoId} (via ${url.split('?')[0]})`,
                    );
                    break;
                  }
                }
      ...
      for (let clickAttempt = 0; clickAttempt < 10; clickAttempt++) {
        ...
        for (let poll = 0; poll < 3; poll++) {
          ...
          if (postBtnGone || successMsg || redirected) {
            log(
              `Post confirmed! (btnGone: ${postBtnGone}, msg: ${!!successMsg}, redirected: ${redirected})`,
            );
            clickedPost = true;
...
export async function runTikTokLogin(profile) {
  ...
  try {
    ...
    if (isLoggedIn) {
      ...
      log(
        'Cookies chÆ°a cÃ³ hoáº·c Ä‘Ã£ háº¿t háº¡n. Vui lÃ²ng báº¥m "Má»Ÿ Profile" Ä‘á»ƒ tá»± Ä‘Äƒng nháº­p trÃªn trÃ¬nh duyá»‡t.',
      );
    ...
    await browser.close().catch(() => null);
    await releaseProfileLocks(userDataDir, profile.name).catch(() => {});
    db.prepare("UPDATE profiles SET status = 'idle' WHERE id = ?").run(
      profileId,
    );
...
export async function addFavoriteMusic(profile, searchTerm) {
  ...
  try {
    ...
    await page
      .evaluate(() => {
        const btn = document.querySelector(
          '.editor-entrance[data-button-name="sounds"], [data-button-name="sounds"], button:has-text("Sounds")',
        );
        if (btn) btn.scrollIntoView({ block: 'center' });
      })
      .catch(() => null);
    ...
    const editBtn = page
      .locator(
        '.editor-entrance[data-button-name="sounds"], button[data-button-name="sounds"], button:has-text("Sounds"), button:has-text("Edit video")',
      )
      .first();
    if (await editBtn.isVisible({ timeout: 15000 })) {
      ...
      const phoneModeGotIt = page
        ...
        .first();
      if (
        await phoneModeGotIt.isVisible({ timeout: 2000 }).catch(() => false)
      ) {
        await phoneModeGotIt.click().catch(() => null);
        await page.waitForTimeout(1000);
      }
      ...
      const searchInput = page
        .locator('input[placeholder*="sound" i], input[placeholder*="music" i]')
        .first();
      if (await searchInput.isVisible({ timeout: 5000 })) {
        ...
        const heartIcon = page
          .locator(
            'div[role="listitem"] [data-icon*="heart"], div[role="listitem"] [data-icon*="bookmark"]',
          )
          .first();

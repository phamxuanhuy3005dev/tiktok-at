...
export function getGroupById(db, id) {
  return db
    .prepare(
      'SELECT id, name, video_folder, created_at FROM groups WHERE id = ?',
    )
    .get(id);
}
...
export function updateGroup(db, id, updates = {}) {
  ...
  if (updates.name !== undefined) {
    ...
    const conflict = db
      .prepare('SELECT id FROM groups WHERE LOWER(name) = LOWER(?) AND id != ?')
  ...
  if (updates.video_folder !== undefined) {
    const folder =
      typeof updates.video_folder === 'string'
        ? updates.video_folder.trim()
        : null;
...
export function deleteGroup(db, id) {
  ...
  assertGroupExists(db, trimmedId);
  const row = db
    .prepare('SELECT COUNT(*) AS n FROM profiles WHERE group_id = ?')
    .get(trimmedId);
  const n = Number(row.n);
  if (n > 0) {
    throw httpError(400, 'Cannot delete group: it still has profiles assigned');
  }

...
router.post('/groups/:id/preview-distribution', (req, res) => {
    try {
        ...
        const folder = req.body.videoFolder || group.video_folder || getConfig('videoFolder', UPLOADS_DIR);
        if (!folder || !fs.existsSync(folder)) {
            return res.status(400).json({ error: `Thư mục video không tồn tại: ${folder || '(chưa chọn)'}` });
        }

        const videos = fs.readdirSync(folder).filter(file => {
            if (file.startsWith('.')) return false;
            const ext = path.extname(file).toLowerCase();
            return ext === '.mp4' || ext === '.mov' || ext === '.webm';
        }).sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));

        const profiles = db.prepare('SELECT id, name, status FROM profiles WHERE group_id = ? ORDER BY name ASC').all(group.id);
...
router.post('/groups/:id/start-automation', async (req, res) => {
    try {
        ...
        const profiles = db.prepare('SELECT * FROM profiles WHERE group_id = ? ORDER BY name ASC').all(group.id);
        if (profiles.length === 0) {
            return res.status(400).json({ error: 'Nhóm này chưa có profile nào.' });
        }
        ...
        executeGroupBatchSession(group, profiles, folder, runMode, limitUploads, uploadLimitCount)
            .catch(err => console.error(`Error in group batch session [${group.name}]:`, err));

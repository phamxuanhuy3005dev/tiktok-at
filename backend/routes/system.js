...
import { selectFolder, clearDebugFiles, clearTrashDirectory } from '../services/system-service.js';
...
router.post('/system/clear-all-temp', (req, res) => {
    try {
        ...
        res.json({
            success: true,
            totalFreedMB: parseFloat(((debugResult.freedBytes + trashResult.freedBytes) / (1024 * 1024)).toFixed(1)),
            debug: debugResult,
            trash: trashResult
        });
    } catch (err) {

...
router.post('/profiles/:id/logout', async (req, res) => {
  ...
  try {
    ...
    db.prepare(
      'UPDATE profiles SET cookies = NULL, status = ? WHERE id = ?',
    ).run('idle', profileId);
    ...
    if (fs.existsSync(networkDir)) {
      try {
        ...
        console.error(
          `[${profile.name}] Failed to clean local cookie files:`,
          e.message,
        );
      }
    }
    ...
    res.json({
      success: true,
      message: 'Đã đăng xuất và xóa session thành công',
    });
  } catch (err) {
...
router.get('/profiles/:id/cookie', async (req, res) => {
  const profile = db
    .prepare('SELECT id, name, cookies FROM profiles WHERE id = ?')
    .get(req.params.id);
  if (!profile) return res.status(404).json({ error: 'Profile not found' });
  ...
  if (openBrowser) {
    try {
      ...
      if (browserCookies && browserCookies.length > 0) {
        const hasSess = browserCookies.some(
          (c) =>
            c.name === 'sessionid' ||
            c.name === 'sessionid_ss' ||
            c.name === 'sid_tt',
      ...
      console.warn(
        `[${profile.name}] Failed to capture cookies from open browser:`,
        e.message,
      );
    }
    ...
    try {
      const diskCookies = await extractCookiesFromDisk(
        profile.name,
        PROFILES_DIR,
      );
      if (diskCookies && diskCookies.length > 0) {
        const hasSess = diskCookies.some(
          (c) =>
            c.name === 'sessionid' ||
            c.name === 'sessionid_ss' ||
            c.name === 'sid_tt',
        ...
        if (hasSess) {
          db.prepare('UPDATE profiles SET cookies = ? WHERE id = ?').run(
            JSON.stringify(diskCookies),
            profile.id,
          );
          profile.cookies = JSON.stringify(diskCookies);
          console.log(
            `[${profile.name}] Auto-recovered ${diskCookies.length} cookies from disk into DB`,
          );
        }
      }
    } catch (e) {
      console.warn(
        `[${profile.name}] Failed to extract cookies from disk:`,
        e.message,
      );
    }
  }
...
router.post('/profiles/:id/cookie', (req, res) => {
  ...
  if (!cookies) {
    db.prepare('UPDATE profiles SET cookies = NULL WHERE id = ?').run(
      profile.id,
    );
...
router.post('/profiles/:id/save-session-cookies', async (req, res) => {
  ...
  if (browser) {
    try {
      ...
      console.warn(
        `[${profile.name}] Error capturing from live browser:`,
        err.message,
      );
    }
  }
  ...
  if (cookies && cookies.length > 0) {
    ...
    return res.json({
      ...
      count: cookies.length,
      cookies,
      message: `Đã lưu ${cookies.length} cookie thành công!`,
    });
  } else {
    return res
      .status(400)
      .json({
        error:
          'Không tìm thấy cookie nào (Hãy mở trình duyệt và đăng nhập TikTok trước)',
      });
  }
});
...
export function importCookiesJsonRecords(database, items) {
  ...
  const byName = new Map(
    existingProfiles.map((p) => [p.name.toLowerCase(), p]),
  );
  ...
  const groupByName = new Map(
    existingGroups.map((g) => [g.name.toLowerCase().trim(), g]),
  );
  ...
  for (const item of items) {
    ...
    if (item.cookies !== undefined && item.cookies !== null) {
      if (typeof item.cookies === 'string') {
        ...
        typeof item.cookies === 'object' &&
        Object.keys(item.cookies).length > 0
      ) {
        cookieStr = JSON.stringify(item.cookies);
      }
    }
    ...
    else if (item.group && typeof item.group.name === 'string')
      groupNameInput = item.group.name;
    else if (typeof item.group_name === 'string')
      groupNameInput = item.group_name;
    groupNameInput = groupNameInput.trim();

    let rawGroupId = '';
    if (typeof item.group_id === 'string') rawGroupId = item.group_id;
    else if (item.group && typeof item.group.id === 'string')
    ...
    if (existing) {
      ...
      try {
        const newProfile = createProfileRecord(database, {
          ...
          set_music:
            item.set_music !== undefined ? (item.set_music ? 1 : 0) : 1,
          remove_title:
            item.remove_title !== undefined ? (item.remove_title ? 1 : 0) : 1,
          need_content_check:
            item.need_content_check !== undefined
              ? item.need_content_check
                ? 1
                : 0
              : 0,
  ...
  try {
    ...
    if (!Array.isArray(items)) {
      if (items && Array.isArray(items.profiles)) {
        ...
        return res
          .status(400)
          .json({ error: 'Dữ liệu phải là một mảng các profile JSON' });
      }
    }

...
export function createProfileRecord(
  ...
  const normalizedRemoveTitle =
    remove_title !== undefined ? (remove_title ? 1 : 0) : 1;
  const normalizedNeedContentCheck =
    need_content_check !== undefined ? (need_content_check ? 1 : 0) : 0;
  ...
  try {
    ...
    if (e && e.code === 'SQLITE_CONSTRAINT_PRIMARYKEY') {
      throw createStoreError('A profile with this id already exists', 400);
    }
    if (e && e.code === 'SQLITE_CONSTRAINT_UNIQUE') {
      const msg = String(e.message ?? '');
      if (msg.includes('profiles.name')) {
        throw createStoreError('A profile with this name already exists', 400);
      }
      if (msg.includes('profiles.id')) {
        throw createStoreError('A profile with this id already exists', 400);
      }
      throw createStoreError('Profile violates a unique constraint', 409);
    }

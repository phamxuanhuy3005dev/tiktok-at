...
const profile = db.prepare('SELECT * FROM profiles WHERE name = ?').get('devyfunkk');
...
const srcVideo = path.join(__dirname, '../../dummy_videos/sample_vertical_1.mp4');
const testVideoPath = path.join(testDir, 'test_vertical.mp4');
fs.copyFileSync(srcVideo, testVideoPath);
console.log(`Copied test video to: ${testVideoPath} (${fs.statSync(testVideoPath).size} bytes)`);
...
try {
    const uploadedCount = await uploadVideo(profile, testDir, videos, false, 0, false);

...
export function computeAutoIncrementTime({
  lastScheduledTime,
  intervalMinutes = 5,
  now = new Date(),
}) {
  const stepMin =
    Number(intervalMinutes) === 10 ? 10 : Number(intervalMinutes) || 5;
  const stepMs = stepMin * 60 * 1000;
  const TWENTY_MINUTES_IN_MS = 20 * 60 * 1000;
  ...
  if (
    lastScheduledTime &&
    lastScheduledTime instanceof Date &&
    !Number.isNaN(lastScheduledTime.getTime())
  ) {
...
export function formatScheduleValue(date, kind, meta = {}) {
  ...
  if (kind === 'date') {
    ...
    if (/mm\s*\/\s*dd/.test(hint) || hint.includes('/'))
...
export function parseTikTokContentTimeText(timeText) {
  ...
  const vnMatch = timeText.match(
    /(\d{1,2})\s*thg\s*(\d{1,2})[\s,]+(\d{1,2}):(\d{2})(?:\s*(AM|PM))?/i,
  );
  if (vnMatch) {
  ...
  const monthMatch = timeText.match(

...
test('formatScheduleValue adapts to date and time field hints', () => {
  ...
  assert.equal(
    formatScheduleValue(date, 'date', { placeholder: 'YYYY-MM-DD' }),
    '2026-04-12',
  );
  assert.equal(
    formatScheduleValue(date, 'date', { placeholder: 'MM/DD/YYYY' }),
    '04/12/2026',
  );
  assert.equal(
    formatScheduleValue(date, 'time', { placeholder: 'HH:mm' }),
    '16:45',
  );
  assert.equal(
    formatScheduleValue(date, 'time', { placeholder: 'hh:mm AM/PM' }),
    '4:45 PM',
  );
});

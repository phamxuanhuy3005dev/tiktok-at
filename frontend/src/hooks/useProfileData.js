...
export const useProfileData = ({
      ...
      if (bRes && bRes.data) {
        ...
        setBatchStatus((prev) => {
          ...
          if (
            prev &&
            newStatus &&
            JSON.stringify(prev) === JSON.stringify(newStatus)
          )
            return prev;
          return newStatus;
        });
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      setConfig((prev) => {
        ...
        if (
          prev.videoFolder === next.videoFolder &&
      ...
      const newStatus =
        bRes && bRes.data && bRes.data.status !== 'idle' ? bRes.data : null;
      setBatchStatus((prev) => {
        if (prev === null && newStatus === null) return null;
        if (
          prev &&
  ...
  const updateGroup = useCallback(
    async (id, updates) => {
      try {
        ...
        if (res.data) {
          setGroups((prev) =>
            prev.map((g) => (g.id === id ? { ...g, ...res.data } : g)),
          );
        }
        await fetchData();
        ...
        setMessage({
          type: 'error',
          text: err.response?.data?.error || 'Không thể cập nhật nhóm',
        });
      }
    },
  ...
  const deleteGroup = useCallback(
    async (id) => {
      if (
        !window.confirm(
          'Bạn có chắc muốn xóa nhóm này? Nhóm phải không còn profile nào gán bên trong.',
        )
      )
        return;
      try {
  ...
  const updateProfileGroup = useCallback(
    async (profileId, groupId) => {
      try {
        ...
        const errText =
          err.response?.data?.error || 'Không thể cập nhật nhóm cho profile';
        setMessage({ type: 'error', text: errText });
      }
    },
    [fetchData, setMessage],
  ...
  const deleteProfile = useCallback(
    async (id) => {
      if (
        !window.confirm(
          'Bạn có chắc muốn xóa profile này? Dữ liệu profile sẽ được đưa vào thùng rác.',
        )
      )
        return;
      try {
  ...
  const deleteSelectedProfiles = useCallback(
    async (targetSet, clearSelectionFn) => {
      const target = targetSet || selectedForRun;
      if (!target || target.size === 0) {
        setMessage({
          type: 'error',
          text: 'Vui lòng chọn ít nhất một profile để xóa.',
        });
        return;
      }
      if (
        !window.confirm(
          `Bạn có chắc muốn xóa ${target.size} profile đã chọn? Việc này cũng sẽ chuyển thư mục profile vào thùng rác.`,
        )
      )
        return;
      try {
        await axios.post('/api/profiles/delete-multiple', {
          profileIds: Array.from(target),
        });
        ...
        setMessage({
          type: 'success',
          text: `Đã xóa thành công ${target.size} profile đã chọn.`,
        });
      } catch (err) {
        console.error(err);
  ...
  const updateConfig = useCallback(async () => {
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: 'Đã lưu cấu hình hệ thống thành công!',
      });
    } catch (err) {
      console.error('Error updating config:', err);
      setMessage({
        type: 'error',
        text: err.response?.data?.error || 'Lỗi khi lưu cấu hình hệ thống',
      });
    } finally {
  ...
  const updateProfileFolder = useCallback(
    async (id, folder) => {
      try {
  ...
  const updateProfileChannelIds = useCallback(
    async (id, channelIds) => {
      try {
  ...
  const updateProfileName = useCallback(
    async (id, newName) => {
      ...
      try {
        ...
        setMessage({
          type: 'error',
          text: err.response?.data?.error || 'Không thể đổi tên profile',
        });
        setEditingId(null);
      }
    },
  ...
  const updateProfileSetMusic = useCallback(
    async (id, enabled) => {
      if (processingRef.current.has(id)) return;
      setProfiles((prev) =>
        prev.map((p) =>
          p.id === id ? { ...p, set_music: enabled ? 1 : 0 } : p,
        ),
      );
  ...
  const updateProfileRemoveTitle = useCallback(
    async (id, enabled) => {
      if (processingRef.current.has(id)) return;
      setProfiles((prev) =>
        prev.map((p) =>
          p.id === id ? { ...p, remove_title: enabled ? 1 : 0 } : p,
        ),
      );
      processingRef.current.add(id);
      try {
  ...
  const updateProfileNeedContentCheck = useCallback(
    async (id, enabled) => {
      if (processingRef.current.has(id)) return;
      setProfiles((prev) =>
        prev.map((p) =>
          p.id === id ? { ...p, need_content_check: enabled ? 1 : 0 } : p,
        ),
      );
      processingRef.current.add(id);
      try {
        await axios.patch(`/api/profiles/${id}`, {
          need_content_check: enabled,
        });
        await new Promise((resolve) => setTimeout(resolve, 500));
        await fetchData();
      } catch (err) {
  ...
  const updateProfileAutoIncrementSchedule = useCallback(
    async (id, enabled) => {
      if (processingRef.current.has(id)) return;
      setProfiles((prev) =>
        prev.map((p) =>
          p.id === id ? { ...p, auto_increment_schedule: enabled ? 1 : 0 } : p,
        ),
      );
      processingRef.current.add(id);
      try {
        await axios.patch(`/api/profiles/${id}`, {
          auto_increment_schedule: enabled,
        });
        await new Promise((resolve) => setTimeout(resolve, 500));
        await fetchData();
      } catch (err) {
  ...
  const updateProfileScheduleInterval = useCallback(
    async (id, interval) => {
      if (processingRef.current.has(id)) return;
      const intervalNum = Number(interval);
      const intervalVal = [5, 10, 15, 20].includes(intervalNum)
        ? intervalNum
        : 5;
      setProfiles((prev) =>
        prev.map((p) =>
          p.id === id ? { ...p, schedule_interval: intervalVal } : p,
        ),
      );
      processingRef.current.add(id);
      try {
        await axios.patch(`/api/profiles/${id}`, {
          schedule_interval: intervalVal,
        });
        await new Promise((resolve) => setTimeout(resolve, 500));
        await fetchData();
      } catch (err) {
  ...
  const updateProfileUploadCount = useCallback(
    async (id, count) => {
      setProfiles((prev) =>
  ...
  const clearTrash = useCallback(
    async (targetSet) => {
      ...
      if (!target || target.size === 0) {
        setMessage({
          type: 'error',
          text: 'Vui lòng chọn ít nhất một profile để dọn dẹp rác.',
        });
        return;
      }
      ...
      setMessage({
        type: 'info',
        text: `Đang dọn rác cho ${profileIds.length} profile...`,
      });
      try {
        const res = await axios.post('/api/profiles/clear-trash', {
          profileIds,
        });
        ...
        if (totalFreedMB > 0) {
          setMessage({
            type: 'success',
            text: `Đã giải phóng ${totalFreedMB} MB từ ${profileIds.length} profile.`,
          });
        } else {
        ...
        setMessage({
          type: 'error',
          text: err.response?.data?.error || 'Lỗi khi dọn rác',
        });
      }
    },
  ...
  const clearDebugFiles = useCallback(async () => {
    if (
      !window.confirm(
        'Xóa toàn bộ file debug PNG và dọn automation.log?\nHành động này không ảnh hưởng đến profile hay cookie.',
      )
    )
      return;
    ...
    try {
      ...
      setMessage({
        type: 'success',
        text: `Đã xóa ${deletedFiles} file debug PNG + dọn log → giải phóng ${freedMB} MB`,
      });
    } catch (err) {
      setMessage({
        type: 'error',
        text: err.response?.data?.error || 'Lỗi khi xóa debug',
      });
    }
  }, [setMessage]);

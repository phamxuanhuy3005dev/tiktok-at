...
const SettingsView = ({
  ...
  const handleClearTrash = async () => {
    if (
      !window.confirm(
        'Bạn có chắc muốn xóa vĩnh viễn toàn bộ profile trong Thùng rác (trash)? Hành động này không thể hoàn tác.',
      )
    ) {
      return;
    }
    ...
    try {
      ...
      notify(
        'success',
        `Đã dọn sạch thùng rác! Giải phóng ${freedMB} MB ổ đĩa.`,
      );
    } catch (err) {
      notify(
        'error',
        `Lỗi dọn thùng rác: ${err.response?.data?.error || err.message}`,
      );
    } finally {
  ...
  const handleClearDebug = async () => {
    ...
    try {
      ...
      notify(
        'success',
        `Đã dọn ${count} file debug screenshot! Giải phóng ${freedMB} MB.`,
      );
    } catch (err) {
      notify(
        'error',
        `Lỗi dọn file debug: ${err.response?.data?.error || err.message}`,
      );
    } finally {
  ...
  const handleClearAllTemp = async () => {
    if (
      !window.confirm(
        'Dọn dẹp toàn bộ thùng rác và file tạm hệ thống để tối ưu dung lượng ổ đĩa?',
      )
    ) {
      return;
    }
    ...
    try {
      ...
      notify(
        'success',
        `Dọn dẹp hoàn tất! Tổng dung lượng đã giải phóng: ${totalFreed} MB.`,
      );
    } catch (err) {
      notify(
        'error',
        `Lỗi dọn dẹp hệ thống: ${err.response?.data?.error || err.message}`,
      );
    } finally {
    ...
    <section>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">
            Thiết lập tham số tự động hóa, tài nguyên và dọn dẹp dung lượng đĩa
          </p>
        </div>
      </div>
        ...
        <div className="settings-stack">
          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <div className="input-with-icon" style={{ flex: 1 }}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setConfig({ ...config, videoFolder: e.target.value })
                  }
            ...
            <p className="input-hint">
              Đường dẫn thư mục chứa video (.mp4, .mov) khi profile không chọn
              thư mục riêng.
            </p>
          </div>
          ...
          <div>
            ...
            <p className="input-hint">
              Số lượng trình duyệt Playwright mở đồng thời khi chạy chế độ song
              song (máy 4-8GB RAM nên để 1-2).
            </p>
          </div>
      ...
      <div className="glass settings-card" style={{ marginTop: '20px' }}>
        ...
        >
          ...
          <h3 style={{ margin: 0, fontSize: '1rem', fontWeight: 600 }}>
            Dọn Dẹp Dung Lượng Đĩa & Thùng Rác (Trash)
          </h3>
        </div>
        <p
          style={{
        ...
        >
          Khi bạn xóa profile, hệ thống chuyển thư mục vào thùng rác{' '}
          <code>trash/</code> để đề phòng xóa nhầm. Bạn có thể xóa sạch các thư
          mục này và file ảnh debug để giải phóng dung lượng ổ đĩa.
        </p>
      ...
      >
        <div className="glass tip-card">
          ...
          />
          <h4 style={{ fontSize: '0.85rem', marginBottom: '4px' }}>
            Mẹo Hiệu Năng Cho Máy Yếu
          </h4>
          <p
            style={{
              fontSize: '0.75rem',
          ...
          >
            Hệ thống đã tự động giới hạn RAM V8 (512MB) và bộ đệm ổ đĩa (32MB)
            cho mỗi profile Chrome. Với máy 4GB-8GB RAM, nên chạy 1-2 profile
            song song hoặc chọn chế độ Tuần tự.
          </p>
        </div>
        <div className="glass tip-card">
          <ShieldCheck
          ...
          >
            Toàn bộ cơ sở dữ liệu được lưu trữ cục bộ trên máy bằng SQLite WAL
            Mode tốc độ cao, không lưu dữ liệu lên cloud bên ngoài.
          </p>
        </div>
      </div>
    </section>

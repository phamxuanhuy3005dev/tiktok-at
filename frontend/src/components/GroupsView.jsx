...
const GroupsView = ({
  ...
  const handleOpenAutomationModal = async (group) => {
    if (!group.profile_count || group.profile_count === 0) {
      notify(
        'error',
        `Nhóm "${group.name}" chưa có profile nào. Vui lòng gán profile vào nhóm trước.`,
      );
      return;
    }
  ...
  const loadDistributionPreview = async (groupId, folder) => {
    ...
    try {
      const res = await axios.post(
        `/api/groups/${groupId}/preview-distribution`,
        { videoFolder: folder },
      );
      ...
      notify(
        'error',
        err.response?.data?.error || 'Không thể xem trước phân chia video',
      );
    } finally {
  ...
  const handleStartGroupAutomation = async () => {
    ...
    try {
      await axios.post(
        `/api/groups/${automationModalGroup.id}/start-automation`,
        {
      ...
      notify(
        'success',
        `Đã kích hoạt tự động hóa nhóm "${automationModalGroup.name}"! Video được chia đều cho các profile.`,
      );
      setAutomationModalGroup(null);
    } catch (err) {
      notify(
        'error',
        err.response?.data?.error || 'Không thể bắt đầu tự động hóa nhóm',
      );
    } finally {
    ...
    <section>
      <div className="page-header" style={{ marginBottom: '20px' }}>
        ...
        <p
          ...
          style={{ maxWidth: '700px', lineHeight: 1.5, marginTop: '6px' }}
        >
          Tạo nhóm và quản lý các kênh theo chủ đề. Bạn có thể chọn thư mục
          video cho cả nhóm, hệ thống sẽ tự động{' '}
          <strong>chia đều video cho từng profile</strong> để upload cùng lúc.
        </p>
      </div>
                ...
                {editingGroupId === g.id ? (
                  ...
                  >
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter')
                          updateGroupName(g.id, editingGroupValue);
          ...
          >
            ...
            >
              <div>
                ...
                >
                  Phân chia đều video cho{' '}
                  {automationModalGroup.profile_count || 0} profile trong nhóm
                </p>
              </div>
              <button
                type="button"
            ...
            >
              ...
              <div>
                ...
                {isLoadingPreview ? (
                    ...
                    >
                      <span>
                        ...
                        <strong style={{ color: '#10B981' }}>
                          {distributionPreview.totalVideos}
                        </strong>{' '}
                        video
                      </span>
                      <span>
                        Số profile nhận video:{' '}
                        <strong style={{ color: 'var(--primary)' }}>
                          {distributionPreview.profilesCount}
                        </strong>{' '}
                        profile
                      </span>
                    </div>
                  ...
                  >
                    Chọn thư mục video ở trên để xem bảng phân bổ đều cho từng
                    profile.
                  </div>
                )}
              </div>
            ...
            >
              ...
              <button
                ...
                disabled={
                  ...
                  !distributionPreview ||
                  distributionPreview.totalVideos === 0
                }
                style={{
                  display: 'flex',
                  alignItems: 'center',
